...
function Home({ darkMode }) {
  ...
  // Fetch warning message using api
  useEffect(() => {
    const fetchWarningMessage = async () => {
      ...
      try {
        ...
        if (!res.data?.message) {
          element.innerHTML = "No gym warning message available.";
          return;
        }
        const message = res.data.message;
        element.innerHTML = '';
        typeWarningMessage(message, element, 100);
      } catch (err) {
        element.innerHTML = "Error loading gym warning.";
        console.error(err);
      }
    };
    fetchWarningMessage();
        ...
        modal.innerHTML = `
          <div class="logout-modal">
            ...
            <div class="logout-buttons">
              <button class="icon-btn" onclick="window.open('https://wa.me/919705635139', '_blank')"><i class="fa-brands fa-square-whatsapp"></i></button>
              <button class="icon-btn" onclick="window.open('https://m.me/16mt9fogu5', '_blank')"><i class="fa-brands fa-square-facebook"></i></button>
              <button class="icon-btn" onclick="window.open('https://www.instagram.com/direct/t/', '_blank')"><i class="fa-brands fa-square-instagram"></i></button>
              <button class="icon-btn" onclick="window.open('mailto:santoshkumarkowru@gmail.com')"><i class="fa-solid fa-square-envelope"></i></button>
            </div>
          </div>
        `;
  ...
  return (
    <div className={darkMode ? 'dark-mode' : ''}>
            ...
            <div
              ...
              onClick={() => {
                if (feature.action) {
                  ...
                  if (['FAQs', 'Testimonials', '24/7 Gym Access'].includes(feature.label)) {
      ...
      <section className="about-section gym-section">
        ...
        <p><strong>SK Fitness</strong> is a state-of-the-art fitness facility offering strength training, cardio, CrossFit, and personal training programs. We combine technology, motivation, and expert guidance to help you reach your goals.</p>
        <p>Whether you're a beginner or a professional athlete, SK Fitness has the tools, trainers, and support system you need to thrive.</p>
      </section>

      {/* About the Website */}
      <section className="about-section website-section">
        <h2><i className="fa-solid fa-globe section-icon"></i> About the Website</h2>
        <p>The SK Fitness digital platform extends your gym experience into your lifestyle. Track workouts, view videos, connect with trainers, and stay motivated — all from your mobile or desktop.</p>
        <ul>
      ...
      <section className="about-section booking-section">
        ...
        <p>At SK Fitness, convenience meets flexibility with our seamless online session booking system. Whether you’re at home, at work, or on the go, you can <strong>reserve your preferred workout slot easily and instantly</strong>.</p>
        <p>No need to visit the gym or make a call—simply use our website or mobile app to select a date and time that suits your schedule. This advanced booking system helps you plan your fitness routine ahead, ensuring your session fits perfectly into your busy lifestyle.</p>
        <p>Our booking platform also helps maintain safe gym capacity limits, so you can enjoy a comfortable workout environment without overcrowding. Plus, with reminders and easy rescheduling options, managing your fitness commitments has never been simpler.</p>
        <p>Experience the freedom to train on your terms and stay committed to your goals with SK Fitness’s hassle‑free online booking.</p>
      </section>

      {/* About the Owner */}
      <section className="about-section owner-section">
        <h2><i className="fa-solid fa-user section-icon"></i> About the Owner</h2>
        <p><strong>Santosh Kumar</strong>, founder of SK Fitness, built this gym and platform with the vision of merging traditional fitness with smart, modern tools. His mission is to make professional fitness guidance accessible to everyone.</p>
        <p>With a background in training and technology, he ensures members get a motivating experience both inside the gym and online.</p>
        ...
        {localStorage.getItem("email") ? (
          <>
            ...
            <p style={{ fontWeight: 600 }}>
              Want to logout? <span onClick={() => setShowLogoutModal(true)} style={{ color: "#007bff", fontWeight: 600, cursor: "pointer", textDecoration: "none" }}>Logout</span>
            </p>
          </>
        ) : (
          <>
            ...
            <p style={{ fontWeight: 600 }}>
              Already have an account? <Link to="/login?tab=signin" state={{ darkMode }} style={{ color: "#007bff", textDecoration: "none", cursor: "pointer", fontWeight: 600, marginRight: "6px" }}>Sign In</Link>
              or <Link to="/login?tab=signup" state={{ darkMode }} style={{ color: "#007bff", textDecoration: "none", cursor: "pointer", fontWeight: 600, marginLeft: "6px" }}>Sign Up</Link> to get started with your personalized training journey today..!
            </p>
